...
class Database:
    ...
    async def register_user(self, telegram_id: int, username: str, password: str, full_name: str = None) -> dict:
        if await self.auth_users.find_one({'username': username}):
        ...
        await self.auth_users.insert_one({
        ...
        if not await self.users.find_one({'user_id': internal_id}):
    ...
    async def login_user(self, telegram_id: int, username: str, password: str) -> dict:
        user = await self.auth_users.find_one({'username': username, 'password': password})
        if user:
            await self.auth_users.update_many(
                {'telegram_id': telegram_id, 'is_logged_in': True},
                {'$set': {'is_logged_in': False}}
    ...
    async def is_user_logged_in(self, telegram_id: int) -> bool:
        return await self.auth_users.find_one({'telegram_id': telegram_id, 'is_logged_in': True}) is not None
    ...
    async def get_internal_user_id(self, telegram_id: int) -> Optional[int]:
        ...
        return auth_user.get('internal_user_id') if auth_user else None
    ...
    async def add_account(self, user_id: int, email: str, tokens: Dict) -> str:
        ...
        if await self.accounts.count_documents({'user_id': user_id}) == 1:
